...
def main():
    ...
    for req in basic_requirements:
        if not run_command(f"{pip_cmd} install {req}", f"Installing {req.split('==')[0]}"):
...
"""
    ...
    if not run_command(f"{python_cmd} -c \"from backend.enhanced_database import init_enhanced_db; init_enhanced_db()\"", "Database initialization"):
